    ...
    function generateDocuments(lc) {
        ...
        return [
            ...
            },
            {
                ...
                checks: [
                    { id: 'ci-invoicedate', text: `Invoice date: extract the date — must fall on or after LC opening date ${fd(lc.issuedDate)} and on or before expiry ${fd(lc.expiryDate)}` },
                    { id: 'ci-total',       text: `Invoice total: extract the final total and confirm it matches exactly ${amt} to the cent — any rounding or discrepancy is a fail` },
                    { id: 'ci-fob',         text: `FOB value: extract the stated FOB line-item figure — must appear as a separate amount, not bundled into the total` },
                    { id: 'ci-freight',     text: `Freight: extract the stated freight amount — must appear as a separate line; FOB + Freight must arithmetically equal the invoice total` },
                    { id: 'ci-unitprice',   text: `Unit price: extract the price per ${g.quantityUnit || 'kg'} and confirm it exactly matches ${lc.currency || 'USD'} ${g.unitPrice || '—'} — a rounding of even one cent is a discrepancy` },
                    { id: 'ci-goods',       text: `Goods description and quantity: extract the goods description wording, stated quantity, and package count — confirm quantity is ${(g.quantity || 0).toLocaleString()} ${g.quantityUnit || 'kg'}, ${g.packageCount || '?'} ${g.packageType || 'packages'}, origin ${g.origin || '?'}` },
                    { id: 'ci-wtinternal',  text: `Net weight internal consistency: find EVERY mention of net weight, total weight, or quantity in the document (goods line, weight summary, packing section, footer — anywhere). Extract each figure and confirm they all agree with each other AND all equal exactly ${(g.quantity || 0).toLocaleString()} ${g.quantityUnit || 'kg'}. A discrepancy between sections — even if the totals look correct — is a fail.` },
                    { id: 'ci-incoterms',   text: `Incoterms: extract exact wording and confirm it matches "${g.incoterms || '—'}" with "Incoterms 2020" — flag any abbreviation (C&F, C&I, CFR without year)` },
                    { id: 'ci-portloading', text: `Port of loading: extract exact wording and confirm it matches "${p.loading || '—'}" — flag any abbreviation or vague wording such as "any port"` },
                    { id: 'ci-lcref',       text: `LC reference: extract what appears and confirm both LC number #${lc.lcNumber} and opening date ${fd(lc.issuedDate)} are stated verbatim` },
                    { id: 'ci-proforma',    text: `Proforma ref: extract the stated reference and date — must exactly match No. ${lc.proformaRef || '—'}${lc.proformaDate ? ' dated ' + fd(lc.proformaDate) : ''} — a wrong reference or date is a common rejection` },
                    { id: 'ci-importer',    text: `Importer name and address: extract what appears and confirm it exactly matches "${ap.name || '—'}${ap.address ? ', ' + ap.address : ''}" — different punctuation, abbreviation, or word order is a discrepancy` },
                    { id: 'ci-importertax', text: `Regulatory numbers: confirm BIN, TIN, IRC for ${ap.name || 'importer'} and TIN/VAT for ${ab.name || 'applicant bank'} all appear; extract the HS code and confirm it matches ${g.hsCode || '—'}` },
                    { id: 'ci-certclause',  text: `Certification clause: extract the exact wording — must read "Goods herein invoiced conform to proforma invoice issued by the beneficiary" — any paraphrase is a flag` },
