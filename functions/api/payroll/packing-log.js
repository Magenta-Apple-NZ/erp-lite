...
// PATCH /api/payroll/packing-log  { entries: [{date, employee, boxes10kg, boxes1kg, notes}] }
...
export async function onRequestPatch({ env, request }) {
    try {
        ...
        for (const e of entries) {
            ...
            if (prev) {
                byKey.set(key, { ...prev,
                    boxes10kg: Number(e.boxes10kg) || 0,
                    boxes1kg:  Number(e.boxes1kg)  || 0,
                    notes:     String(e.notes || ''),
                });
            } else {
                const id = 'pack-' + String(seq++).padStart(4, '0');
